"""PostgreSQL implementation of the IPRange read model repository."""
...
class PostgresIPRangeReadModelRepository(IPRangeReadModelRepository):
    """PostgreSQL-backed read model repository for IP range queries and projections."""
    ...
    async def find_by_id(self, entity_id: UUID) -> dict | None:
        """Return an IP range dict by primary key, or None if not found or deleted."""
    ...
    async def find_all(
        ...
        *,
        ...
        """Return a paginated list of IP range records matching the given filters."""
