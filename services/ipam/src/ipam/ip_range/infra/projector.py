...
from ipam.ip_range.domain.events import IPRangeCreated, IPRangeStatusChanged, IPRangeUpdated
...
async def handle_ip_range_updated(session_factory, cache, event: DomainEvent) -> None:
    ...
    if values:
        await _update_model(session_factory, IPRangeReadModel, event.aggregate_id, values)
